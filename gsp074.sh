...
echo "${CYAN_TEXT}${BOLD}    10th Game: Skills Boost Arcade Certification Zone April 2025   ${RESET_FORMAT}"
echo "${CYAN_TEXT}${BOLD}           1st Lab: Cloud Storage: Qwik Start - CLI/SDK           ${RESET_FORMAT}"
echo "${CYAN_TEXT}${BOLD}----------------------------------------------------------------${RESET_FORMAT}"
echo ""

# 🌍 Fetching Region
echo "${MAGENTA_TEXT}${BOLD}${UNDERLINE_TEXT} 🔄 Fetching Region...${RESET_FORMAT}"
export REGION=$(gcloud compute project-info describe \
--format="value(commonInstanceMetadata.items[google-compute-default-region])")

# 🗺️ Fetching Zone
echo "${MAGENTA_TEXT}${BOLD}${UNDERLINE_TEXT} 🔄 Fetching Zone...${RESET_FORMAT}"
export ZONE=$(gcloud compute project-info describe \
--format="value(commonInstanceMetadata.items[google-compute-default-zone])")

# 🆔 Fetching Project ID
echo "${MAGENTA_TEXT}${BOLD}${UNDERLINE_TEXT} 🔍 Fetching Project ID...${RESET_FORMAT}"
PROJECT_ID=`gcloud config get-value project`

# 🔢 Fetching Project Number
echo "${MAGENTA_TEXT}${BOLD}${UNDERLINE_TEXT} 🔍 Fetching Project Number...${RESET_FORMAT}"
export PROJECT_NUMBER=$(gcloud projects describe $PROJECT_ID --format="value(projectNumber)")

# 💡 Start-Up Banner
echo "${CYAN_TEXT}${BOLD}-------------------------------------------------------${RESET_FORMAT}"
echo "${CYAN_TEXT}${BOLD}         🚀 INITIATING THE TASK EXECUTION...          ${RESET_FORMAT}"
echo "${CYAN_TEXT}${BOLD}-------------------------------------------------------${RESET_FORMAT}"
echo ""
...
echo ""
echo -e "${WHITE_TEXT}${BOLD}YouTube Channel:${RESET_FORMAT} ${BLUE_TEXT}${UNDERLINE}https://www.youtube.com/@TechXNinjas${RESET_FORMAT}"
