...
echo "${CYAN_TEXT}${BOLD_TEXT}         Google Kubernetes Engine: Qwik Start       ${RESET_FORMAT}"
echo "${CYAN_TEXT}${BOLD_TEXT}-------------------------------------------------------${RESET_FORMAT}"
echo ""
...
echo
echo "${BLUE_TEXT}${BOLD_TEXT}---> Creating the GKE cluster 'lab-cluster'...${RESET_FORMAT}"
gcloud container clusters create --machine-type=e2-medium --zone=$ZONE lab-cluster

echo
echo "${BLUE_TEXT}${BOLD_TEXT}---> Fetching credentials for 'lab-cluster'...${RESET_FORMAT}"
gcloud container clusters get-credentials lab-cluster

echo
echo "${BLUE_TEXT}${BOLD_TEXT}---> Deploying the 'hello-server' application to your cluster...${RESET_FORMAT}"
